...
void pachube_in_out(){
  ...
  if (ready_to_update){
    ...
    if (localClient.connect()) {
      ...
      float verdi= Thermister(analogRead(0));
      sprintf(pachube_data,"%d,%d,%d,%d,%d,%d,%d,%d",verdi,analogRead(1),analogRead(2),analogRead(3),analogRead(4),analogRead(5), successes + 1, failures);
